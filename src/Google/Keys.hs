...
newtype ReCaptchaSiteKey = ReCaptchaSiteKey
  ...
  } deriving (Eq, Show, FromJSON, ToJSON, Arbitrary)
...
data ReCaptchaVerifyResponse
  = ReCaptchaVerifyResponse Bool
  deriving (Eq, Show)

instance Arbitrary ReCaptchaVerifyResponse where
  arbitrary = oneof
    [ ReCaptchaVerifyResponse <$> arbitrary
    ]
